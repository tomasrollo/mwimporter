	...
	mwimporter.Views.FilecontrolView = Backbone.View.extend({
		...
		downloadResultFile: function() {
			...
			var csv = mwimporter.records.chain().filter(function(r) {
				...
				return true; // export all transfers by default
			}).map(function(r) {
				var record = r.toJSON();
				record.transfers = mwimporter.accounts.get(record.transfers).get('name');
				record.category = mwimporter.categories.get(record.category).get('name');
				...
				if (record.transfers === 'Cash') { // create an inverse transaction for cash withdrawals
					var addedRecord = _.clone(record);
					addedRecord.transfers = addedRecord.account;
					addedRecord.account = 'Cash';
					addedRecord.amount *= -1;
					addedTrns.push(addedRecord);
				}
				
				return tpl(record);
			}).value();
			var csv2 = _(addedTrns).map(function(record) { return tpl(record); });
